...
> **分支说明**：本分支 `hatch-pet` = 人物线，角色为橘猫 + 图集少女
> ×4（二乃/莉莉艾/小光/露莎米奈）+ 芒果图集猫 + 竹兰照片精灵；
> 宝可梦（皮卡丘/伊布/谢米/比克提尼/新叶喵 + 叫声交互 + 专属随机
> 动作）在 `pokemon` 分支。运行时代码两分支共享，角色内容不同。
三次元明星 6 位已按需求移除
...
## 3.1 宝可梦图集流程（2026-09-13，生成配方备查）

> 宝可梦角色本体在 `pokemon` 分支；本节保留生成管线配方，
> 在人物线上供未来扩展其他图集角色参考。
...
- `hatch-pet`：**人物线**（本分支）——图集桌宠管线 + 播放器 +
  芒果/二乃/莉莉艾/小光/露莎米奈图集 + 竹兰照片精灵 + 本地 ComfyUI
  生图后端 + 交互/气泡/菜单重构，基于 main
- `pokemon`：**宝可梦线**——基于 hatch-pet，只保留 5 只宝可梦图集
  （皮卡丘/伊布/谢米/比克提尼/新叶喵）+ 叫声交互 + 专属随机动作；
  人物内容已移除，运行时代码两分支共享
